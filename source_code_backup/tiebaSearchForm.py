...
class TiebaForm(FlaskForm):
    keyword = StringField(label='搜索关键词', validators=[DataRequired("请输入关键词")], description="请输入关键词",
                          render_kw={"required": "required"})
    page = StringField(label='筛选前几页', validators=[DataRequired("请输入页数")], description="请输入页数",
                       render_kw={"required": "required"})
